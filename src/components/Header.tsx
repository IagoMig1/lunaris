      ...
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        <div className="flex items-center">
          <span className="text-[#292929] text-2xl font-black tracking-wider">
            LUNÁRIS<span className="text-blue-500">.</span>
          </span>
        </div>
        {/* Desktop Navigation */}
        <nav className="hidden md:flex items-center space-x-8">
          {['Início', 'Serviços', 'Sobre', 'Contato'].map(item => <a key={item} href={`#${item.toLowerCase()}`} className="text-[#292929] font-medium hover:text-purple-500 transition-colors">
              {item}
            </a>)}
        </nav>
        {/* Mobile Menu Button */}
